...
function tste_liviza_icon_libraries(){
	
	/**
	There are three steps to add new icon library:
	
	1. Add library in this array
	2. Upload icon library files in /themestek-liviza/extras/icon-picker/icon-libraries/ folder.
	3. Create new JS array file (iconset file) as /themestek-liviza/extras/icon-picker/iconset/icon-LIBRARY.js file.
	...
	*/
	...
	$icon_library = array(
		...
		'fontawesome' => array( esc_attr__( 'FontAwesome', 'liviza' ),		'fa fa-ok',					'/font-awesome/css/font-awesome.min.css' ),
		'themify'     => array( esc_attr__( 'Themify icons', 'liviza' ),		'ti-location-pin',			'/themify-icons/themify-icons.css' ),
		'sgicon'      => array( esc_attr__( 'Skotroke Gap Icon', 'liviza' ),	'sgicon sgicon-WorldWide',	'/stroke-gap-icons/style.css' ),
		'vc_linecons' => array( esc_attr__( 'Linecons', 'liviza' ),			'li_star', 					'/vc-linecons/vc_linecons_icons.min.css' ),
		'themestek_liviza_business_icon' => array( esc_attr__( 'Liviza Special Icon', 'liviza' ),	'themestek-liviza-business-icon-checklist', 					'/themestek-liviza-icons/flaticon.css' ),
	);
 ...
 */
if( !function_exists('tste_liviza_admin_iconpicker_scripts_styles') ){
function tste_liviza_admin_iconpicker_scripts_styles() {
	
	$icon_libraries = tste_liviza_icon_libraries();
	
	
	/** JS Files - Icon library array files **/
	foreach( $icon_libraries as $icon_id=>$icon_data ){
		wp_enqueue_script( 'iconset-'.$icon_id, THEMESTEK_LIVIZA_URI . '/icon-picker/iconset/iconset-'.$icon_id.'.js', array( 'bootstrap' ) );
	}
	...
	foreach( $icon_libraries as $icon_id=>$icon_data ){
		...
		wp_enqueue_style( $icon_id, THEMESTEK_LIVIZA_URI.'/icon-picker/icon-libraries'.$icon_css ); // from plugin
	}
	...
	wp_enqueue_script( 'bootstrap', THEMESTEK_LIVIZA_URI . '/libraries/bootstrap/js/bootstrap.min.js', array( 'jquery' ) );
	
	// Bootstrap icon picker
	wp_enqueue_script( 'bootstrap-iconpicker', THEMESTEK_LIVIZA_URI . '/libraries/bootstrap-iconpicker/js/bootstrap-iconpicker.js', array( 'bootstrap' ) );
	
	// Bootstrap icon picker - CSS
	wp_enqueue_style( 'bootstrap-iconpicker', THEMESTEK_LIVIZA_URI . '/libraries/bootstrap-iconpicker/css/bootstrap-iconpicker.min.css' );
	
	// icon picker
	wp_enqueue_script( 'themestek-icon-picker', THEMESTEK_LIVIZA_URI . '/icon-picker/icon-picker.js', array( 'jquery' ) );
	
	
}
}
add_action( 'admin_enqueue_scripts', 'tste_liviza_admin_iconpicker_scripts_styles' );
 ...
 */
if( !function_exists('tste_liviza_icon_libraries_css') ){
function tste_liviza_icon_libraries_css() {
...
add_action( 'wp_enqueue_scripts', 'tste_liviza_icon_libraries_css' );
 ...
 */
if( !function_exists('tste_liviza_admin_iconpicker_library') ){
function tste_liviza_admin_iconpicker_library(){
	?>
				...
				for (var i = 0; i < length; i++) {
					...
					for (var j = 0; j < op.cols; j++) {
						...
						if (pos < op.icons.length) {
							...
							var spaninner = $('<span class="themestek-icon-tooltip"><i class="' + op.iconClass + ' ' + v + '"></i></span>');
...
add_action('admin_head', 'tste_liviza_admin_iconpicker_library');
