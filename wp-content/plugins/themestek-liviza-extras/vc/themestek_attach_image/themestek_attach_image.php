...
function themestek_attach_image_settings_field( $settings, $value, $tag, $single = true  ) {
	...
	$output .= '<div class="themestek_image_selector_w">';
	$output .= '<input type="hidden" class="wpb_vc_param_value themestek_gallery_widget_attached_image_val '
	           . $settings['param_name'] . ' '
	...
	$output .= '<li><div class="themestek_inner"><img src="'.$thumb_img.'" /> <a href="#" class="themestek_vc_icon-remove"><i class="vc-composer-icon vc-c-icon-close"></i></a> </div></li>';
	$output .= '</ul>';
	...
	$output .= '</div>';
	if ( true === $single ) {
		$output .= '<a class="themestek_gallery_widget_add_images" href="#" use-single="true" title="'
		...
		$output .= '<a class="themestek_gallery_widget_add_images" href="#" title="'
	...
	$output .= '</div> <!-- .themestek_image_selector_w --> ';
	
	return $output;
	
}
vc_add_shortcode_param( 'themestek_attach_image', 'themestek_attach_image_settings_field', THEMESTEK_LIVIZA_URI . '/vc/themestek_attach_image/themestek_attach_image.js' );
