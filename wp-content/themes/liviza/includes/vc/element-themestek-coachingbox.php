...
$coachingCatList = array();
if( taxonomy_exists('themestek-coaching-category') ){
	$coachingCatList_data = get_terms( 'themestek-coaching-category', array( 'hide_empty' => false ) );
	$coachingCatList      = array();
	foreach($coachingCatList_data as $cat){
		$coachingCatList[ esc_html($cat->name) . ' (' . esc_html($cat->count) . ')' ] = esc_html($cat->slug);
	}
}
// Getting Options
...
$allParams = array_merge(
		...
		array(
			array(
				...
				'param_name'	=> 'boxstyle',
				'std'			=> 'style-1',
				'value'			=> themestek_global_template_list('coaching', false),
		...
		array( 
			array(
				...
				"description" => sprintf( esc_html__("Show sortable category links above %s items so user can sort by just single click.",'liviza'), $ch_type_title_singular ),
				"param_name"  => "sortable",
				"value"       => array(
			...
			array(
				...
				"description" => sprintf( esc_html__("Show pagination links below %s boxes.",'liviza'), $ch_type_title ),
			...
			array(
				"type"        => "checkbox",
				"heading"     => sprintf( esc_html__("From %s", "liviza"), $ch_cat_title_singular ),
				"description" => sprintf( esc_html__('If you like to show %1$s from selected %2$s than select the category here.', 'liviza'), $ch_type_title, $ch_cat_title ),
				"param_name"  => "category",
				"value"       => $coachingCatList,
				'group'		  => esc_html__( 'Box Options', 'liviza' ),
			),
			array(
				"type"        => "dropdown",
				...
				"description" => esc_html__("Sort retrieved coaching by parameter.",'liviza'),
				"param_name"  => "orderby",
				"value"       => array(
			...
			array(
				...
				"description" => sprintf( esc_html__("How many %s item you want to show.", "liviza"), $ch_type_title ),
				"param_name"  => "show",
				"value"       => array(
...
$themestek_sc_params_coachingbox = $params;
vc_map( array(
	"name"     => sprintf( esc_html__("ThemeStek %s Box",'liviza'), $ch_type_title_singular ),
	"base"     => "themestek-coachingbox",
	"class"    => "",
