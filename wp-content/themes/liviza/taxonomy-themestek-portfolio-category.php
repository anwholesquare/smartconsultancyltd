...
get_header(); ?>
	<section id="primary" class="content-area <?php echo themestek_sanitize_html_classes(themestek_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($liviza_theme_options['pfcat_view']) ) ? $liviza_theme_options['pfcat_view'] : 'overlay' ;
			$column = ( !empty($liviza_theme_options['pfcat_column']) ) ? $liviza_theme_options['pfcat_column'] : 'three' ;
			?>	
